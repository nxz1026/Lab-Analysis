...
|----------|--------|-------------|
| 7 stat charts + alerts | PNG + JSON | Trend / correlation / inflammation / anomaly / MA / CV / Z-score |
| Literature evidence report | Markdown | PubMed top-k + 5-dim grading + LLM interpretation |
| Imaging consistency report | Markdown | MRI × lab cross-validation (Qwen-VL) |
| Integrated clinical report | Markdown / JSON / FHIR | 9 sections + optional FHIR R4 Bundle |
| **Quant eval report** | **PNG + HTML** | **6 metrics std vs dspy auto-scoring + viz** |
| **Cross-run trend** | **PNG** | **Longitudinal trend across runs for same patient** |
| Scoring card & diagnosis | JSON + MD | 5-dim 0-100 + top-3 hypotheses |

LLM agents (Claude Desktop / Cursor) can invoke the full pipeline via **MCP server 6 tools** directly.
