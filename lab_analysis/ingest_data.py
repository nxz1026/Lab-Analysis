...
def process_batch(
    items: list, process_func: callable, batch_mode: bool, item_name: str = "项"
) -> tuple:
    """
    通用批量处理函数
...
def ingest_lab_image(image_path: Path, patient_id: str, report_date: str, report_type: str) -> dict:
    """摄入检验报告图片"""
...
def ingest_mri_dicom(
    zip_path: Path = None, dicom_dir: Path = None, patient_id: str = None, report_date: str = None
) -> dict:
    """摄入MRI DICOM数据"""
    from lab_analysis.pipeline.cli import get_deid
...
def main():
    ...
    parser.add_argument(
        "--path", "-p", type=Path, nargs="*", help="文件路径列表（lab_image/mri_report，支持多个）"
    )
    parser.add_argument(
        "--zip-path", type=Path, nargs="*", help="DICOM ZIP文件路径列表（mri_dicom，支持多个）"
    )
    parser.add_argument(
        "--dicom-dir", type=Path, nargs="*", help="DICOM已解压目录路径列表（mri_dicom，支持多个）"
    )
        ...
        if args.type == "lab_image":
            ...
            def process_lab_image(image_path):
                ...
                record = ingest_lab_image(
                    image_path, patient_id, args.report_date, args.report_type
                )
                print(f"[OK] 检验报告摄入成功: {record['saved_path']}")
                return record

            success_count, fail_count = process_batch(
                paths, process_lab_image, batch_mode, "检验报告"
            )
            ...
            def process_dicom_source(source_item):
                ...
                if source_type == "zip":
                    record = ingest_mri_dicom(
                        zip_path=source_path, patient_id=patient_id, report_date=args.report_date
                    )
                else:
                    record = ingest_mri_dicom(
                        dicom_dir=source_path, patient_id=patient_id, report_date=args.report_date
                    )
            ...
            success_count, fail_count = process_batch(
                all_sources, process_dicom_source, batch_mode, "DICOM数据"
            )
            ...
            success_count, fail_count = process_batch(
                paths, process_mri_report, batch_mode, "MRI报告"
            )
