...
def check_patient_data(patient_id: str) -> bool:
    """检查病人原始数据目录是否存在且有内容。"""
    raw_dir = WIKI_ROOT / "raw" / patient_id
    lab_dir = raw_dir / "lab"
...
def run_step(name: str, script: Path, extra_args: list[str] = None) -> int:
    """运行单步脚本。"""
...
def main():
    ...
    if not check_patient_data(patient_id):
        ...
        print(f"   /root/wiki/raw/{patient_id}/lab/        ← 检验报告截图")
        print(f"   /root/wiki/raw/{patient_id}/papers/    ← 结构化报告（lab_report_*/metrics.md）")
        print(f"   /root/wiki/raw/{patient_id}/imaging/   ← MRI 影像序列（可选）")
    ...
    # Step 1: 数据加载
    rc = run_step("② 数据加载 (data_loader)", SCRIPTS_DIR / "data_loader.py", pid_arg)
    if rc != 0:
        print(f"[!] data_loader 失败，退出")
        sys.exit(1)

    # Step 2: 数据分析
    rc = run_step("③ 数据分析 (data_analyzer)", SCRIPTS_DIR / "data_analyzer.py", pid_arg)
    if rc != 0:
        print(f"[!] data_analyzer 失败，退出")
        sys.exit(1)

    # Step 3: 文献检索
    rc = run_step("④ 文献检索 (literature_searcher)", SCRIPTS_DIR / "literature_searcher.py", pid_arg)
    if rc != 0:
    ...
    if args.skip_llm:
        ...
        rc = run_step("⑤ 循证解读 (literature_interpreter)", SCRIPTS_DIR / "literature_interpreter.py", pid_arg)
        if rc != 0:
    ...
    if args.skip_imaging:
        ...
        rc = run_step("⑥ 影像分析 (qwen_vl_report_check)", SCRIPTS_DIR / "qwen_vl_report_check.py", pid_arg)
        if rc != 0:
            print(f"[!] qwen_vl_report_check 失败（非致命，继续）")

    # Step 6: 生成最终报告
    rc = run_step("⑦ 生成最终报告 (gen_final_report)", SCRIPTS_DIR / "gen_final_report.py", pid_arg)
    if rc != 0:
        print(f"[!] gen_final_report 失败（非致命，继续）")

    # Step 7: 飞书上云
    rc = run_step("⑧ 飞书上云 (upload_to_feishu)", SCRIPTS_DIR / "upload_to_feishu.py", pid_arg)
    if rc != 0:
