...
def collect_mri_samples():
    ...
    for ts_dir in sorted(data_root.iterdir()):
        ...
        report_findings = mri_data.get('report_findings', '') or \
            mri_data.get('paper_report_text', '') or \
            "胰腺体尾部萎缩，伴主胰管不规则扩张，符合慢性胰腺炎表现"
        ...
        for r in mri_data.get('results', []):
            ...
            if isinstance(analysis, list):
                # 取每个 dict 的 text 字段
                analysis_text = '\n'.join(item.get('text', '') if isinstance(item, dict) else str(item)
            ...
            if '### 1. 解剖定位' in analysis_text or '解剖定位' in analysis_text:
                ...
                if m:
                    al = m.group(1).strip()
                m2 = re.search(r'影像所见[：:]\s*(.+?)(?=###|印证评价|补充发现|总结|$)', analysis_text, re.DOTALL)
                if m2:
                    imf = m2.group(1).strip()[:500]
                m3 = re.search(r'印证评价[：:]*\s*(.+?)(?=###|补充发现|总结|$)', analysis_text, re.DOTALL)
                if m3:
