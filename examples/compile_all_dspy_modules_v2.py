            ...
            for r in mri_data.get("results", []):
                if r.get("status") == "success":
                    s["mri_samples"].append({
                        ...
                        "imaging_findings": r.get("analysis", "") or r.get("imaging_findings", ""),
            ...
            for lr_dir in sorted(patient_raw.glob("lab_report_*")):
                ...
                if metrics_md.exists() and metadata_md.exists():
                    s["lab_samples"].append({
                        ...
                        "physician": "李文",
                        "diagnosis": "慢性胰腺炎",
                        "wbc_value": 6.5, "wbc_unit": "10^9/L", "wbc_ref_range": "4.0-10.0",
                        "rbc_value": 4.5, "rbc_unit": "10^12/L", "rbc_ref_range": "3.5-5.5",
                        "hgb_value": 140.0, "hgb_unit": "g/L", "hgb_ref_range": "110-160",
                        "plt_value": 200.0, "plt_unit": "10^9/L", "plt_ref_range": "100-300",
                        "crp_value": 8.5, "crp_unit": "mg/L", "crp_ref_range": "<10",
                        "hs_crp_value": 2.5, "hs_crp_unit": "mg/L", "hs_crp_ref_range": "<3.0",
                        "confidence": 0.85,
...
def compile_final_report(samples):
    ...
    for s in samples:
        if not s.get("final_report_md"):
            continue
        raw.append({
            "patient_info": {"name": s["patient_id"], "age_sex": "未知", "exam_id": "MRI-2026-04-11"},
    ...
    raw = ensure_min_examples(raw, 3)
    trainset = to_examples(raw, ['patient_info', 'lab_summary', 'analysis_results',
...
def main():
    ...
    results = {}
    for fn in [compile_literature_interpreter, compile_final_report,
               compile_mri_analyzer, compile_lab_extractor]:
        try:
            r = fn(samples)
    ...
    for name in ['compile_literature_interpreter', 'compile_final_report',
                 'compile_mri_analyzer', 'compile_lab_extractor']:
