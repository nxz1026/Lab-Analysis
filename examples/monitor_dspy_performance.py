...
def evaluate_interpretation_quality(interpretation: str) -> Dict:
    ...
    # 检查结构化
    section_keywords = ["病理生理", "临床意义", "建议", "机制", "治疗", "诊断"]
    ...
    elif metrics["length_score"] in ["excellent", "good"] and metrics["structure_score"] in [
        "excellent",
        "good",
    ]:
...
def compare_modes(samples: List[Dict]) -> Dict:
    ...
    results = {"standard": [], "dspy": [], "improvement": {}}
    ...
    if standard_stats and dspy_stats:
        length_improvement = (
            (dspy_stats["avg_length"] - standard_stats["avg_length"])
            / standard_stats["avg_length"]
            * 100
        )
        score_improvement = (
            (dspy_stats["avg_score"] - standard_stats["avg_score"])
            / standard_stats["avg_score"]
            * 100
        )
